...
#@+node:gcross.20100923134429.1869: *3* n2l
#@+at
# Utility function converting numbers to letters.
#@@c
n2l = map(chr,range(ord('A'),ord('Z')+1))
#@+node:gcross.20100923134429.1870: *3* make_contractor
def make_contractor(tensor_index_labels,index_join_pairs,result_index_labels,name="f"):    # pre-process parameters
    ...
    #@+node:gcross.20100923134429.1871: *4* << def build_statements >>
    ...
    return f
#@+node:gcross.20100923134429.1875: *3* make_contractor_from_implicit_joins
...
#@+node:gcross.20100923134429.1877: *3* crand
def crand(*shape):
    return rand(*shape)*2-1+rand(*shape)*2j-1j
#@+node:gcross.20100923134429.1879: *3* form_contractor
